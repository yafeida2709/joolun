...
public class LhPackageApi {
	 ...
	 * 通过运营商查询对应的套餐
	 * */
	@GetMapping("/operatorId/{operatorId}" )
	public R packageQueryThroughOperator(@PathVariable("operatorId") String operatorId){
		LhPackage lhPackage = new LhPackage();
		lhPackage.setOperatorId(operatorId);
		return R.ok(lhPackageService.list(Wrappers.query(lhPackage)));
	}
